...
public class DriverFactory {
	 ...
	 * SeleniumTestParameters testParameters) { MobileWebDriver driver = null; try {
	 * switch (testParameters.getExecutionMode()) {
	 * 
	 * case SEETEST: driver = SeeTestDriverFactory .getSeeTestDriver(
	 * mobileProperties.getProperty("SeeTestHost", "localhost"),
	 * Integer.parseInt(testParameters .getSeeTestPort()), mobileProperties
	 * .getProperty("SeeTestProjectBaseDirectory"), mobileProperties.getProperty(
	 * "SeeTestReportType", "xml"), "report", "Test Name from Driver Init",
	 * testParameters.getMobileExecutionPlatform(), mobileProperties
	 * .getProperty("SeeTestAndroidApplicationName"), mobileProperties
	 * .getProperty("SeeTestiOSApplicationName"), mobileProperties
	 * .getProperty("SeeTestAndroidWebApplicationName"), mobileProperties
	 * .getProperty("SeeTestiOSWebApplicationName"),
	 * testParameters.getDeviceName()); break;
	 * 
	 * default: throw new Exception("Unhandled Execution Mode!"); } } catch
	 * (Exception ex) { ex.printStackTrace(); log.error(ex.getMessage()); } return
	 * driver; }
	 */
