...
import com.CucumberCraft.pageObjects.TestPOM;
...
public class LoginStepDefs extends MasterStepDefs {
	...
	TestPOM testPOM = new TestPOM();
	
	
	@When("^I search using the valid origin \"([^\"]*)\" and the valid destination \"([^\"]*)\"$")
	public void i_search_using_the_valid_origin_and_the_valid_destination(String from, String to) throws Throwable {
		testPOM.searchFlight(from, to);
	}
	
	@When("^I login using the invalid username (.*) and the invalid password (.*)$")
	public void i_login_using_invalid_username_invalid_password(
			String userName, String password) {
		login(userName, password);
	}

	@When("^I login using the valid username (.*) and the invalid password (.*)$")
	public void i_login_using_valid_username_invalid_password(String userName,
			String password) {
		login(userName, password);
	}

	@When("^I login using the valid username (.*) and the valid password (.*)$")
	public void i_login_using_valid_username_valid_password(String userName,
			String password) {
		login(userName, password);
	}
	...
	public void application_should_stay_on_login_page() {
		...
		assertTrue(driver.getTitle().contains("Sign-on")
				|| driver.getTitle().contains("Welcome"));
	}
